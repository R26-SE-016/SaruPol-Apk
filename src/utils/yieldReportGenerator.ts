...
export async function exportAllFarmsReportPDF(farms: Farm[]): Promise<void> {
  const dateStr = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  ...
  farms.forEach((f) => {
    totalPerches += (f.perches || 0);
    totalTrees += (f.totalTrees || 0);
    rows += `<tr>
      <td>${f.name}</td>
  ...
  const html = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>Global Farms Summary</title>
<style>
  @page { margin: 16mm; }
  body { font-family: -apple-system, 'Segoe UI', Roboto, sans-serif; color: #1e293b; margin: 0; padding: 24px; }
  .header { background: #1B4D3E; color: #fff; border-radius: 16px; padding: 24px 28px; margin-bottom: 24px; }
  ...
  <div class="header">
    <h1>Global Farms Summary Report</h1>
    <div style="font-size: 12px; opacity: 0.85;">Generated on ${dateStr}</div>
  </div>
  
  <div style="margin-bottom: 20px;">
    <h3>Total Portfolio</h3>
    <p><strong>Total Farms:</strong> ${farms.length}</p>
    <p><strong>Total Area:</strong> ${totalPerches} perches</p>
    <p><strong>Total Trees:</strong> ${totalTrees}</p>
  </div>

  <h3>Farm Breakdown</h3>
  <table>
    <thead><tr><th>Farm Name</th><th>Location</th><th>Area (Perches)</th><th>Trees</th><th>IoT Connected</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>
  
  <p style="text-align:center;font-size:10px;color:#94a3b8;margin-top:40px">Generated by Coconut Farm Telemetry &middot; ${dateStr}</p>
</body></html>`;
